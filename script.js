...
async function loadSurah(surahNumber) {
  ...
  // جلب السورة من API
  const surah = await fetchQuranData(surahNumber);
  if (!surah) {
    content.innerHTML = '<div class="error">تعذر تحميل نص السورة.</div>';
    return;
  }
  content.innerHTML = `
    <div class="surah-header">
      <h3>${surah.name}</h3>
      <p>السورة رقم ${surah.number} - ${surah.numberOfAyahs} آية</p>
    </div>
    <div class="ayah-container">
      ${surah.ayahs.map(ayah => `<div class="ayah">${ayah.text} (${ayah.numberInSurah})</div>`).join('')}
    </div>
  `;
}

// ===== وظائف الأحاديث =====
function showHadithCategory(category, event) {
  ...
  // تحديث الأزرار (معالجة الحدث بشكل صحيح)
  buttons.forEach(btn => btn.classList.remove('active'));
  if (event && event.target) event.target.classList.add('active');
  else if (buttons[0]) buttons[0].classList.add('active'); // افتراضياً أول زر
...
// ===== وظائف الأدعية =====
function showDuaCategory(category, event) {
...
function incrementMainCounter() {
  ...
  if (currentZikr === 'سبحان الله') {
    count('subhanallah');
  } else if (currentZikr === 'الحمد لله') {
    count('alhamdulillah');
  } else if (currentZikr === 'الله أكبر') {
    count('allahu_akbar');
  } else if (currentZikr === 'لا إله إلا الله') {
    count('la_ilaha');
  } else if (currentZikr === 'اللهم صل على محمد') {
    count('salat');
  } else if (currentZikr === 'أستغفر الله') {
    count('istighfar');
  }
...
function count(zikrType) {
  if (!counters.hasOwnProperty(zikrType)) return;
  counters[zikrType]++;
  const el = document.getElementById(zikrType);
  if (el) el.textContent = counters[zikrType];
...
// تصدير الوظائف للاستخدام العام// ===== البيانات والمحتوى =====
...
window.count = count;// ===== البيانات والمحتوى =====
...
window.searchSurah = searchSurah;
window.showHadithCategory = showHadithCategory;
window.showDuaCategory = showDuaCategory;// ===== البيانات والمحتوى =====
